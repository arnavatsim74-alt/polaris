...
export function RouteImportMapping({ parsedRoutes, onComplete, onCancel }: RouteImportMappingProps) {
  ...
  const uniqueRanks = useMemo(
    () =>
      [
        ...new Set(parsedRoutes.map((r) => r.min_rank).filter(Boolean).filter((rank) => !normalizeRank(rank!))),
      ] as string[],
    [parsedRoutes]
  );
    ...
    const mappedRoutes = parsedRoutes.map((route) => {
      ...
      const mappedLiveryValues = rawAircraftValues
        .map((value, index) => {
          ...
          const fallbackLivery = aircraft?.find(
            (ac) => String(ac.icao_code || "").toUpperCase() === String(mappedIcao || "").toUpperCase() && ac.livery
          )?.livery;
  ...
  return (
    <Card className="max-w-2xl mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <Dialog open={Boolean(activeAircraftCsv)} onOpenChange={(open) => !open && setActiveAircraftCsv(null)}>
          <DialogContent className="p-0 sm:max-w-[500px]">
            <DialogHeader className="px-4 pt-4 pb-1">
              <DialogTitle>Select aircraft mapping{activeAircraftCsv ? ` for "${activeAircraftCsv}"` : ""}</DialogTitle>
            </DialogHeader>
            {activeAircraftCsv && (
              <Command>
                <CommandInput placeholder="Search aircraft..." />
                <CommandList>
                  ...
                  <CommandGroup>
                    {uniqueIcaoCodes.map((icao) => (
                      ...
                      >
                        <Check
                          className={`mr-2 h-4 w-4 ${activeSelectedIcao === icao ? "opacity-100" : "opacity-0"}`}
                        />
                        {aircraftByIcao[icao]?.name || icao} ({icao})
                      </CommandItem>
                    ))}
          ...
          <div className="space-y-4">
            ...
            {uniqueRanks.map((csvRank) => (
              <div key={csvRank} className="space-y-2 p-3 bg-muted/50 rounded-lg">
                ...
                <Select value={rankMappings[csvRank] || ""} onValueChange={(v) => handleRankChange(csvRank, v)}>
                  <SelectTrigger>
        ...
        <p className="text-xs text-center text-muted-foreground">{parsedRoutes.length} routes ready to import</p>
      </CardContent>
    </Card>
  );
}
